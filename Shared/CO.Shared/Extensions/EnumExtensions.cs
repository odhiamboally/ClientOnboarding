...
namespace CO.Shared.Extensions;
...
public static class EnumExtensions
{
    ...
    public static List<LookupResponse> ToLookupResponses<T>() where T : struct, Enum
    {
        return [.. Enum.GetValues<T>().Select(e => new LookupResponse(
            Id: Convert.ToInt32(e),
            Name: e.ToString(),
            Description: e.ToDisplayString() 
            
        ))];
    }

    public static List<(T Value, string Display)> GetSelectList<T>()
        where T : struct, Enum
